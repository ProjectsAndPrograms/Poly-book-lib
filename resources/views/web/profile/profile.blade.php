                            ...
                            <div class="container mt-4">
                                <div>
                                    <nav>
                                        <div class="nav nav-tabs mb-3">
                                            <button class="nav-link  active" type="button" role="tab"
                                                ...
                                                aria-controls="nav-about" aria-selected="true">Downloads</button>
                                            <button class="nav-link " type="button" role="tab" id="nav-mission-tab"
                                                data-bs-toggle="tab" data-bs-target="#nav-mission"
                                                aria-controls="nav-mission" aria-selected="false">Feedbacks</button>

                                            @if ($user->transactions && count($user->transactions) > 0)
                                                <button class="nav-link " type="button" role="tab" id="nav-transactions-tab"
                                                    data-bs-toggle="tab" data-bs-target="#nav-transactions"
                                                    aria-controls="nav-transactions" aria-selected="false">Transactions</button>
                                            @endif
                                        </div>
                                    </nav>
                                    <div class="tab-content mb-5">
                                        ...
                                        @if ($user->transactions && count($user->transactions) > 0)
                                            <div class="tab-pane" id="nav-transactions" role="tabpanel"
                                                aria-labelledby="nav-transaactions-tab">
                                                @include('web.profile.user_transactions')
                    ...
                    <div class="col-lg-4 order-lg-2" data-aos="fade-up" data-aos-delay="100">
                        <div class="position-relative p-0">
                            <div class="profile-img">
                                ...
                                <form action="{{ route('profile.image_update', $user) }}" method="POST"
                            ...
                            <div class="d-flex justify-content-center">
                                <form action="{{ route('logout') }}" method="POST">
                                    ...
                                    <button class="btn btn-outline-danger d-flex align-items-center">
                                        <iconify-icon icon="solar:logout-2-line-duotone"
                                            class="fs-4 fw-bold me-2"></iconify-icon>
                                        Logout
                                    </button>
                                </form>
                            </div>
